...
try {
    ...
    $statusEntry = [pscustomobject]@{
        ...
        CleanCsvPath = $cleanCsvPath
        FinalOutputCsvPath = $(if ($ilasStatus -eq "SUCCESS" -or $SkipIlasStep) { $csvPath } else { "" })
    ...
    if ($ilasStatus -eq "SUCCESS" -or $SkipIlasStep) {
    ...
    if (-not $SkipIlasStep -and $ilasStatus -ne "SUCCESS") {
        $runStatus = "FAILED"
