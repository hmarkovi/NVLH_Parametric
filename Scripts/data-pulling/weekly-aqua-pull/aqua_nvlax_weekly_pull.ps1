...
- ILAS opergroup filtering: Delegated to aqua_nvlax_ilas_vmin_analysis.ps1 with default OpergroupFilter="6248_CLASSHOT"
- ILAS test name cleaning: Added suffix filter (aqua_nvlax_ilas_vmin_analysis.ps1) to remove rows ending in "_it" or "_scrb"
- Retention policy: 7-day test-end lookback maintained (LastNDaysTestEnd=7 as default)

CHANGES (2026-06-09):
- Removed -LastNDaysTestEnd from ILAS script invocation (aqua_nvlax_ilas_vmin_analysis.ps1 no longer receives it)
...
param(
    ...
    [string]$ProgramPattern = "NVLAX6*",
    [string]$Operations = "6248",
    [string]$OutputDirectory = "\\ger\ec\proj\ha\mmgbd\MMGBD_PSA\Products\NVL\NVLAZ Weekly",
...
function Resolve-AquaExePathForAutomation {
    ...
    $cacheDir = Join-Path $env:LOCALAPPDATA "NVLAX\AquaCmdLine"
...
function Get-IlasScriptPath {
    ...
    $candidatePaths = @(
        (Join-Path $weeklyDir "aqua_nvlax_ilas_vmin_analysis.ps1"),
        (Join-Path $scriptsDir "parametric-analysis\ilas\aqua_nvlax_ilas_vmin_analysis.ps1")
    )
...
function Get-MergeScriptPath {
    ...
    $scriptsDir = Split-Path (Split-Path $weeklyDir -Parent) -Parent
    $candidatePaths = @(
        (Join-Path $weeklyDir "merge-ilas-into-upsvf.ps1"),
        (Join-Path $scriptsDir "utility\merge-ilas-into-upsvf.ps1")
    )
...
try {
    ...
    elseif (-not [string]::IsNullOrWhiteSpace($Operations)) {
        $lotArgs = @("-lotsfromfs")
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($RawInputFile)) {
        ...
        if ($lotArgs.Count -gt 0) { $activeFilters += "LotArgs=$($lotArgs -join ' ')" }
