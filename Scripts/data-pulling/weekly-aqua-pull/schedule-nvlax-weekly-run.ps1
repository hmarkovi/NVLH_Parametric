<#
.SYNOPSIS
Create Windows Task Scheduler task to run weekly NVLAX UPSVF+ILAS automation every Sunday at 5:00 AM.

.DESCRIPTION
Sets up a recurring scheduled task that:
1. Runs aqua_nvlax_weekly_pull.ps1 every Sunday at 5:00 AM
...
Task name: "NVLAX UPSVF Weekly Pull"
...
param(
    [string]$WeeklyScriptPath = (Join-Path $PSScriptRoot "aqua_nvlax_weekly_pull.ps1"),
    [string]$IlasScriptPath = (Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "parametric-analysis\ilas\aqua_nvlax_ilas_vmin_analysis.ps1"),
    [string]$OutputDirectory = "\\ger\ec\proj\ha\mmgbd\MMGBD_PSA\Products\NVL\NVLAZ Weekly",
    [string]$TaskName = "NVLAX UPSVF Weekly Pull",
...
Write-Host "Setting up Windows Task Scheduler for weekly NVLAX UPSVF+ILAS automation..."
